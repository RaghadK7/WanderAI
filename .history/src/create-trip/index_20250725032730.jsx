...
import { db, auth } from '@/service/firebaseConfig'; // Import auth
import { 
 ...
 * Displays animated loading screen while processing
 */
...
function CreateTrip() {
  ...
  // Form and UI state
  const [place, setPlace] = useState();              // Selected destination
  const [formData, setFormData] = useState({});      // All form data
  const [openDialog, setOpenDialog] = useState(false); // Show auth dialog
  const [loading, setLoading] = useState(false);     // Loading state
  const [loadingMessage, setLoadingMessage] = useState('');     // Loading title
  const [loadingSubMessage, setLoadingSubMessage] = useState(''); // Loading subtitle

  // Authentication state
  const [isLogin, setIsLogin] = useState(true);      // Toggle login/register
  const [authLoading, setAuthLoading] = useState(false); // Auth loading state
  const [authData, setAuthData] = useState({         // Auth form data
    email: '',
    password: '',
    name: ''
  });
  ...
  /**
   * Updates form data when user makes selections
   * Used for trip preferences (destination, budget, etc.)
   */
   ...
   * Used for email/password fields
   */
   ...
   * Saves trip data to both Firebase and localStorage
   * Now includes userId for better data organization
   */
  const saveTrip = async (tripDataObj) => {
    ...
    const docId = Date.now().toString(); // Unique timestamp ID
    
    try {
      // Enhanced trip document structure
      const tripDocument = {
        // User identification (dual approach for compatibility)
        userEmail: user.email,
        userId: user.uid,           // Firebase UID for better security
   ...
   * Handles authentication check, validation, AI generation, and saving
   */
  const OnGenerateTrip = async () => {
    // Check if user is authenticated (works for both auth methods)
    const user = localStorage.getItem('user');
    if (!user) {
      setOpenDialog(true); // Show authentication dialog
      return;
    }
    ...
    if (errors.length > 0) {
      errors.forEach((error, index) => 
        setTimeout(() => toast.error(error), index * 500) // Stagger error messages
      );
      return;
    }
    ...
    try {
      // Generate trip plan using AI service
      ...
      if (result?.itinerary) {
        ...
        // Save trip after successful generation
   ...
   * Fetches user profile and stores user data
   */
  const GetUserProfile = (tokenInfo) => {
    ...
    .then((resp) => {
      ...
      setOpenDialog(false); // Close auth dialog
      setTimeout(OnGenerateTrip, 1000); // Continue with trip generation
    })
    .catch(() => {
  ...
  /**
   * Renders option cards for budget and travelers
   * Creates interactive selection cards with animations
   */

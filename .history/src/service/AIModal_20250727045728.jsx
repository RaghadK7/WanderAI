...
export const generateTravelPlan = async (location, days, traveler, budget, retryCount = 0) => {
  ...
  try {
    ...
    try {
      ...
      console.log("First parse failed, trying to extract JSON...");
      try {
        // Try to find JSON in the response
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
          let cleanJson = jsonMatch[0].replace(/```json\n?/g, '').replace(/```\n?/g, '');
        ...
        console.log("Second parse also failed, creating fallback response...");
        parsedResponse = {
          hotels: [],
          itinerary: []
        };
    ...
    if (!parsedResponse) {
      parsedResponse = { hotels: [], itinerary: [] };
    }
    ...
    console.log(`Parsed response has ${parsedResponse.itinerary.length} days`);

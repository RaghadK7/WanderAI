    ...
    return (
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-2">{trip?.userSelection?.location?.label || "Unknown Location"}</h3>
            <div className="flex flex-wrap gap-2 mb-2">
              {trip?.userSelection?.categories?.map((cat, idx) => (
                <span key={idx} className="bg-cyan-100 text-cyan-700 px-3 py-1 rounded-full text-xs font-medium">{cat}</span>
              ))}
            </div>
            <div className="text-gray-500 text-sm mb-2">
              {formatDate(trip?.createdAt)}
            </div>
            <div className="text-gray-700">{trip?.userSelection?.description}</div>
          </div>
        </div>
      </div>
    );
